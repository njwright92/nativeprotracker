import { ADD_ITEM, DELETE_ITEM, UPDATE_ITEM, ADD_ENTRY, DELETE_ENTRY, EDIT_ENTRY, ADD_NOTE, EDIT_NOTE } from '../actions/types';
...
const itemsReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case EDIT_NOTE:
            const { itemId: editNoteItemId, noteId: editNoteId, note: editedNote } = action.payload;
            const noteEditItem = state.find((item) => item.id === editNoteItemId);
            if (noteEditItem && noteEditItem.notes) {
                const editTargetNote = noteEditItem.notes.find((n) => n.id === editNoteId);
                if (editTargetNote) {
